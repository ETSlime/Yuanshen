...
RWStructuredBuffer<SmokeParticle> g_ParticlesUAV : register(SLOT_UAV_PARTICLE); // 書き込み用パーティクルデータバッファ
StructuredBuffer<SmokeParticle> g_ParticlesSRV : register(SLOT_SRV_PARTICLE); // 読み取り用パーティクルデータバッファ
...
void UpdateCS(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    SmokeParticle p = g_ParticlesUAV[id];
...
void EmitCS(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    SmokeParticle p = (SmokeParticle) 0; // 明示的に全フィールドを初期化
    ...
    // 色（静的、必要ならfadeをPSで処理）
    p.color = float4(0.6f, 0.6f, 0.6f, 0.4f);
...
{
    SmokeParticle p = g_ParticlesSRV[input[0].ParticleID];
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        o.Color = p.color * (p.lifeRemaining / p.life); // 死にかけで透明になる
        stream.Append(o);
